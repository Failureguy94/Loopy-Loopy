...
/// @title Aave Data Provider Interface (for reserve data)
...
/// @title LooperVault
/// @notice Automated leveraged looping strategy using Reactive Network
/// @dev Handles supply/borrow/swap loops with comprehensive edge case handling
contract LooperVault is Ownable, ReentrancyGuard {
    ...
    function deposit() external payable nonReentrant {
        ...
        emit LoopRequested(msg.sender, depositAmount, targetLTV, block.timestamp);
    }

    // ============ Reactive Callback - Execute Loop Step ============
    
    /// @notice Execute one loop step: borrow -> swap -> supply
    /// @dev Called by Reactive Network callback proxy
    function executeLoopStep(address user) external onlyCallbackProxy nonReentrant {
        ...
        if (pos.loopsCompleted >= absoluteMaxLoops) {
            pos.isLooping = false;
            emit LoopingCompleted(user, pos.currentLTV, pos.loopsCompleted, pos.totalCollateral, pos.totalDebt);
            return;
        }
        ...
        if (currentLTV >= targetLTV) {
            ...
            emit LoopingCompleted(user, currentLTV, pos.loopsCompleted, pos.totalCollateral, pos.totalDebt);
            return;
        }
        ...
        // Check borrow cap
        (uint256 borrowCap, ) = IPoolDataProvider(POOL_DATA_PROVIDER).getReserveCaps(USDC);
        if (borrowCap > 0) {
            (, , , , uint256 totalVariableDebt, , , , , , , ) = 
                IPoolDataProvider(POOL_DATA_PROVIDER).getReserveData(USDC);
            if (totalVariableDebt + borrowAmount > borrowCap * 1e6) {
                pos.isLooping = false;
                emit LoopFailed(user, "BORROW_CAP_EXCEEDED", pos.loopsCompleted);
                revert BorrowCapExceeded(borrowCap * 1e6, totalVariableDebt + borrowAmount);
            }
        }
        ...
        // Get updated LTV
        (totalCollateralBase, totalDebtBase, , , , ) = IPool(AAVE_POOL).getUserAccountData(address(this));
        uint256 newLTV = totalCollateralBase > 0 
            ? (totalDebtBase * PRECISION) / totalCollateralBase 
            : 0;
        pos.currentLTV = newLTV;
        
        emit LoopStepCompleted(user, pos.loopsCompleted, borrowAmount, wethReceived, wethReceived, newLTV);
        ...
        if (newLTV >= targetLTV) {
            pos.isLooping = false;
            emit LoopingCompleted(user, newLTV, pos.loopsCompleted, pos.totalCollateral, pos.totalDebt);
    ...
    function executeUnwind(address user) external onlyCallbackProxy nonReentrant {
        Position storage pos = positions[user];
        if (!pos.isActive) revert NoActivePosition();
        ...
        while (totalDebtBase > 0 && iterations < absoluteMaxLoops) {
            ...
            // Withdraw WETH
            uint256 withdrawn = IPool(AAVE_POOL).withdraw(WETH, withdrawAmount, address(this));
            ...
            uint256 usdcReceived = ISwapRouter(UNISWAP_ROUTER).exactInputSingle(params);
            ...
            // Update debt
            (, totalDebtBase, , , , healthFactor) = IPool(AAVE_POOL).getUserAccountData(address(this));
            iterations++;
        }
        
        // Withdraw remaining collateral
        (uint256 totalCollateralBase, , , , , ) = IPool(AAVE_POOL).getUserAccountData(address(this));
        if (totalCollateralBase > 0) {
            uint256 remainingWeth = IPool(AAVE_POOL).withdraw(WETH, type(uint256).max, address(this));
    ...
    function _swapWithSlippageProtection(uint256 usdcAmount) internal returns (uint256) {
        ...
        uint256 minWeth = (expectedWeth * (PRECISION - maxSlippage)) / PRECISION;
        ...
        uint256 wethReceived = ISwapRouter(UNISWAP_ROUTER).exactInputSingle(params);
    ...
    function _calculateSafeWithdraw(uint256 currentHealthFactor) internal view returns (uint256) {
    ...
    function getCurrentLTV() external view returns (uint256) {
        (uint256 collateral, uint256 debt, , , , ) = IPool(AAVE_POOL).getUserAccountData(address(this));
    ...
    function needsMoreLoops(address user) external view returns (bool) {
        ...
        return pos.currentLTV < targetLTV && pos.loopsCompleted < absoluteMaxLoops;
    }
