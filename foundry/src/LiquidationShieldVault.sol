...
/// @notice Automated liquidation protection vault using Reactive Network
/// @dev Origin contract deployed on Sepolia - creates positions and handles protection
contract LiquidationShieldVault is Ownable, ReentrancyGuard {
    ...
    function createPosition() external payable nonReentrant {
        ...
        (, , uint256 availableBorrows, , , ) = IPool(AAVE_POOL).getUserAccountData(address(this));
        ...
        emit PositionCreated(msg.sender, collateralAmount, borrowAmount, healthFactor);
    ...
    function triggerProtection(address user) external onlyCallbackProxy nonReentrant {
        Position storage pos = positions[user];
        require(pos.isActive, "No active position");
        
        (, , , , , uint256 healthFactor) = IPool(AAVE_POOL).getUserAccountData(address(this));
        require(healthFactor < PROTECTION_THRESHOLD, "Health factor still safe");
        ...
        pos.debtAmount = pos.debtAmount > usdcReceived ? pos.debtAmount - usdcReceived : 0;
        
        // Get new health factor
        (, , , , , uint256 newHealthFactor) = IPool(AAVE_POOL).getUserAccountData(address(this));
